...
function PLUGIN:InitializedPlugins()
	...
	do
		...
		for name, data in pairs(self.ammoInfo) do
			...
			ITEM.maxQuantity = ammoInfo.amount or 30
		...
		for k, v in ipairs(weapons.GetList()) do
			...
			elseif (class:find("ma85_")) then
				prefix = "ma85_"
			elseif (class:find("heretic_")) then
				prefix = "heretic_"
			end
	...
	do	
		...
		CustomizableWeaponry.canDropWeapon = true
		CustomizableWeaponry.enableWeaponDrops = true
